...
Documentation of settings
   All changes are made using /sh_set [-q] [-e] <variable> [value]
	...
	msg_format     : what should be used for message formating, similar to above
			...
			nick_format also has (and is recommended): %nick%
			msg_format also has (and is recommended) : %message% 
...
sub msg_highlight {
	...
	if ($CONF->{nick_format} ne '%nick%') {
		$msgdata[0] = chan_format_nick($msgdata[0]);
	}
	if ($CONF->{msg_format} ne '%message%') {
		$msgdata[1] = chan_format_msg($msgdata[1]);
	}
...
sub macro_chan_sub ($$) {
	...
	elsif ($_[0] eq '~default~')        { return ${$_[1]}; }
	return '%'.$_[0].'%';
}

sub chan_format_nick {
	my $nick = shift;
	my $string = $CONF->{nick_format};
	$string =~ s/%nick%/%~default~%/g;
	$string =~ s/%([^%\s]+)%/macro_chan_sub $1, \$nick/eg;
	return ($string);
}

sub chan_format_msg {
	my $msg = shift;
	my $string = $CONF->{msg_format};
	$string =~ s/%message%/%~default~%/g;
	$string =~ s/%([^%\s]+)%/macro_chan_sub $1, \$msg/eg;
	return ($string);
}
